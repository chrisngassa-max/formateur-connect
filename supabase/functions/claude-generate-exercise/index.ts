...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { skill_type, format, level, theme, gabarit_id, nombre_items, difficulte, formateur_id } = await req.json();
    ...
    const userMessage = `Gabarit : ${JSON.stringify({
      ...
      consignes_generation: (gabarit.structure as any)?.consignes_generation,
      lexique_cibles: (gabarit.structure as any)?.lexique_cibles,
      objectif_principal: (gabarit.structure as any)?.objectif_principal,
    })}
...
Exercices existants à ne pas reproduire : ${JSON.stringify(existingContext)}`;
    ...
    // 4. Valider la structure JSON retournée
